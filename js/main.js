...
let data = { standards: standards, places: [] };

Promise.all([
    fetch('data/width_data.csv').then(r => r.text()),
    fetch('data/step_data.csv').then(r => r.text()),
    fetch('data/slope_data.csv').then(r => r.text()),
    fetch('data/places_info.csv').then(r => r.text())
]).then(([widthCSV, stepCSV, slopeCSV, placesCSV]) => {
    const widthData = parseCSV(widthCSV);
    const stepData = parseCSV(stepCSV);
    const slopeData = parseCSV(slopeCSV);
    const placesInfo = parseCSV(placesCSV);
    
    const placeMap = {};
    widthData.forEach(row => {
        const place = row['장소'];
        if (!placeMap[place]) placeMap[place] = { 유효폭: [], 단차: [], 기울기: [] };
        placeMap[place].유효폭.push(parseFloat(row['유효폭']));
    });
    stepData.forEach(row => {
        const place = row['장소'];
        if (!placeMap[place]) placeMap[place] = { 유효폭: [], 단차: [], 기울기: [] };
        placeMap[place].단차.push(parseFloat(row['단차']));
    });
    slopeData.forEach(row => {
        const place = row['장소'];
        if (!placeMap[place]) placeMap[place] = { 유효폭: [], 단차: [], 기울기: [] };
        placeMap[place].기울기.push(parseFloat(row['기울기']));
    });
    
    placesInfo.forEach(info => {
        const placeName = info['장소'];
        const measurements = placeMap[placeName] || { 유효폭: [], 단차: [], 기울기: [] };
        const avgWidth = calculateAverage(measurements.유효폭);
        const avgStep = calculateAverage(measurements.단차);
        const avgSlope = calculateAverage(measurements.기울기);
        const widthDeviation = avgWidth ? avgWidth - standards.유효폭_m : null;
        const stepDeviation = avgStep ? avgStep - standards.단차_cm : null;
        const slopeDeviation = avgSlope ? avgSlope - standards.기울기_도 : null;
        const widthCompliant = avgWidth ? (avgWidth >= standards.유효폭_m ? "적합" : "부적합") : null;
        const stepCompliant = avgStep ? (avgStep <= standards.단차_cm ? "적합" : "부적합") : null;
        const slopeCompliant = avgSlope ? (avgSlope <= standards.기울기_도 ? "적합" : "부적합") : null;
        const measurementCount = measurements.유효폭.length + measurements.단차.length + measurements.기울기.length;
        
        data.places.push({
            "장소": placeName, "상태": info['상태'], "측정지점수": measurementCount,
            "유효폭": { "값": avgWidth, "편차": widthDeviation, "적합": widthCompliant },
            "기울기": { "값": avgSlope, "편차": slopeDeviation, "적합": slopeCompliant },
            "단차": { "값": avgStep, "편차": stepDeviation, "적합": stepCompliant },
            "위치": info['위치'],
            "점자블록상태": info['점자블록상태'] || null,
            "포장상태상태": info['포장상태상태'] || null,
            "건물진입로상태": info['건물진입로상태']
        });
    });
...
function drawBoxPlotsFromCSV(widthData, stepData, slopeData) {
    const boxMX = [], boxMY = [], boxMPlaces = [];
    const boxCmX = [], boxCmY = [], boxCmPlaces = [];
    const boxDegX = [], boxDegY = [], boxDegPlaces = [];
    
    widthData.forEach(row => {
        boxMX.push('유효폭');
        boxMY.push(parseFloat(row['유효폭']));
        boxMPlaces.push(row['장소']);
    });
    stepData.forEach(row => {
        boxCmX.push('단차');
        boxCmY.push(parseFloat(row['단차']));
        boxCmPlaces.push(row['장소']);
    });
    slopeData.forEach(row => {
        boxDegX.push('기울기');
        boxDegY.push(parseFloat(row['기울기']));
        boxDegPlaces.push(row['장소']);
    });
    
    Plotly.newPlot('boxChartM', [{
        x: boxMX, y: boxMY, type: 'box', name: '데이터',
        boxpoints: 'all', jitter: 0.3, pointpos: -1.8,
        marker: { color: '#42A5F5', size: 6 },
        text: boxMPlaces, hoveron: 'boxes+points',
        ...
        shapes: [{ type: 'line', x0: -0.5, x1: 0.5, y0: 1.2, y1: 1.2, line: { color: 'red', width: 2, dash: 'dash' } }],
        annotations: [{ x: 0, y: 1.2, text: '법적기준: 1.2m', showarrow: false, yshift: 10, font: { color: 'red', size: 10 } }]
    }, { displaylogo: false });
    
    if (boxCmY.length > 0) {
        Plotly.newPlot('boxChartCm', [{
            x: boxCmX, y: boxCmY, type: 'box', name: '데이터',
            boxpoints: 'all', jitter: 0.3, pointpos: -1.8,
            marker: { color: '#AB47BC', size: 6 },
            text: boxCmPlaces, hoveron: 'boxes+points',
            ...
            margin: { t: 40, b: 60, l: 50, r: 20 },
            font: { family: 'Pretendard, -apple-system, sans-serif' },
            shapes: [{ type: 'line', x0: -0.5, x1: 0.5, y0: 2.0, y1: 2.0, line: { color: 'red', width: 2, dash: 'dash' } }],
            annotations: [{ x: 0, y: 2.0, text: '법적기준: 2cm', showarrow: false, yshift: 10, font: { color: 'red', size: 10 } }]
        }, { displaylogo: false });
    }
    
    if (boxDegY.length > 0) {
        Plotly.newPlot('boxChartDeg', [{
            x: boxDegX, y: boxDegY, type: 'box', name: '데이터',
            boxpoints: 'all', jitter: 0.3, pointpos: -1.8,
            marker: { color: '#FF7043', size: 6 },
            text: boxDegPlaces, hoveron: 'boxes+points',
            ...
            margin: { t: 40, b: 60, l: 50, r: 20 },
            font: { family: 'Pretendard, -apple-system, sans-serif' },
            shapes: [{ type: 'line', x0: -0.5, x1: 0.5, y0: 4.8, y1: 4.8, line: { color: 'red', width: 2, dash: 'dash' } }],
            annotations: [{ x: 0, y: 4.8, text: '법적기준: 4.8도', showarrow: false, yshift: 10, font: { color: 'red', size: 10 } }]
        }, { displaylogo: false });
    }
}
...
function initializeCharts() {
        ...
        function updateStatusStats() {
            ...
            document.getElementById('statusGood').textContent = statusCounts['양호'];
            document.getElementById('statusMid').textContent = statusCounts['개선(장기)'];
            document.getElementById('statusUrgent').textContent = statusCounts['긴급조치'];
        }
        
        function drawHeatmap(sortBy = 'none') {
            ...
            metrics.forEach(metric => {
                ...
                places.forEach(place => {
                    const details = placeDetails[place.장소];
                    let items = details && details[metric.key] ? details[metric.key] : [];
                    
                    if (metric.filter === 'slope') {
                        items = items.filter(item => item.설명.includes('기울기'));
                    } else if (metric.filter === 'step') {
                        items = items.filter(item => item.설명.includes('단차'));
                    }
                    ...
                    if (items.length === 0) {
                        ...
                        items.forEach(item => {
                            const cls = item.상태 === '양호' ? 'good' : item.상태.includes('긴급') ? 'urgent' : 'mid';
        ...
        function initPlaceButtons() {
            const northContainer = document.getElementById('north-places');
            const southContainer = document.getElementById('south-places');
            
            northOrder.forEach(displayName => {
            ...
            southOrder.forEach(displayName => {
                const place = findPlaceByDisplayName(displayName);
                if (place) {
                    const btn = document.createElement('button');
                    btn.className = 'place-btn';
            ...
            const firstPlace = findPlaceByDisplayName(northOrder[0]);
...
const sbValues = [
    ...
    if (label === "양호") {
        ...
        html += "<p style='margin-top:15px; color:#1f883d; font-weight:600;'>이 구역들은 현재 접근성이 양호합니다.</p>";
        ...
        html += "</ul>";
        html += "<p style='margin-top:15px; color:#fb8500; font-weight:600;'>체계적인 개선 계획 수립이 권장됩니다.</p>";
        ...
        html += "</ul>";
        html += "<p style='margin-top:15px; color:#cf222e; font-weight:600;'>우선순위를 두고 신속한 개선 조치가 필요합니다.</p>";
